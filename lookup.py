...
def getLoginBar():
    return requests.get(
        "http://openid-proxy.default.svc.cluster.local:9023/_loginBar",
        headers={
...
def getUser():
    ...
    username = db['user'].find_one({'_id': agent
                                    })['username'] if agent else None
...
def proposedUri():
    ...
    return {
        'description':
        prevDoc['description'] if prevDoc else pageTitle.pageTitle(uri),
        'tag':
        prevDoc['tag'] if prevDoc else '',
        'extended':
        prevDoc['extended'] if prevDoc else '',
        'shareWith':
        prevDoc.get('shareWith', []) if prevDoc else [],
...
def tagFilterComplete():
    ...
    for t in allTags(params.user, withTags=haveTags):
        ...
        out.append({
            ...
            t['label'],
            'text':
            "%s (%s%s)" % (t['label'], t['count'], " left" if haveTags else "")
        })
...
def userAddLink(user):
    ...
    doc = links.fromPostdata(bottle.request.params, user,
...
def userLinks(user, tags):
    ...
    data['desc'] = ("%s's recent links" % user) + (" tagged %s" %
                                                   (tags, ) if tags else "")
    data['toRoot'] = siteRoot()
